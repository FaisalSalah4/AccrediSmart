...
export default function Register() {
  ...
  const [form, setForm] = useState({
    name: '', email: '', password: '', role: 'faculty', department: DEPARTMENTS[0],
  })
  ...
  // Department field shown only for non-admin roles
  ...
  const handleRoleChange = (e) => {
    ...
    setForm(f => ({
      ...f,
      role,
      department: role === 'admin' ? '' : (f.department || DEPARTMENTS[0]),
    }))
  }
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true)
    try {
      await signUp({ ...form, department: needsDept ? form.department : '' })
      navigate('/')
    } catch (err) {
        ...
        <div className="bg-white rounded-2xl shadow-2xl p-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              ...
              <input required className="input" placeholder="Dr. Ahmed Al-Rashidi"
                value={form.name} onChange={set('name')} />
            </div>

            <div>
              <label className="label">Email</label>
              <input type="email" required className="input" placeholder="you@university.edu.sa"
            ...
            <div className={needsDept ? 'grid grid-cols-2 gap-3' : ''}>
              <div>
                <label className="label">Role</label>
                <select className="input" value={form.role} onChange={handleRoleChange}>
                  {ROLES.map(r => (
